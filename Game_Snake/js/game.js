...
const fieldSize = { //параметры должны делиться на размер ячейки без остатка
  x: parseInt(canvas.offsetWidth), //принимает строку в качестве аргумента и возвращает целое число
...
let score = 0; //Начальное количество очков - ноль
...
function soundEat() {
  audioEat.currentTime=0.5; //текущая позиция в секундах воспроизведения
  audioEat.play();
}
...
function changeDirection(EO) { 
  EO = EO || window.event; //кроссбраузерность
...
//Функции запрета напрвления в противоположную сторону
function dirUp(EO) {
  EO = EO || window.event;
  EO.preventDefault();
  if (dir != 'down') {
...
function drawSnake() {
  for (let i = snake.length -1; i >= 0; i--) {
    context.beginPath();
    context.fillStyle = (i == 0) ? '#0a1da2' : '#e5e40d';
    context.shadowBlur = 0;
    context.shadowOffsetX = 0;
    context.shadowOffsetY = 0;
    /*создаем круг(голову змейки) x - координата X центра круга
                                  y - координата Y центра круга
                                  r - радиус круга
                                  sAngle - начальный угол в радианах
                                  eAngle - конечный угол в радианах
                                  anticlockwise - рисовка по часовой стрелки(false)
     */
    context.arc(snake[i].x + cell.quarter * 2, snake[i].y + cell.quarter * 2,
                cell.quarter * 2, 0, Math.PI*2, false);
...
Очистка области пикселей внутри поля
          X - координата X левого верхнего угла очищаемой области
          Y - координата Y левого верхнего угла очищаемой области
  ...
  (function () {
    try { //Если происходит ошибка при генерации еды, она ловится и генерация запускается снова. (На всякий случай)
      ...
      context.shadowBlur = 20; //уровень размытости
      context.shadowOffsetX = 1; //горизонтальное расстояние тени от фигуры
      context.shadowOffsetY = 2; //вертикальное расстояние тени от фигуры
  ...
  snake.unshift(newHead); //Добавляем голову в начало массива змеи
}


let game = setInterval(renderingСontrol, 1000 / 60); //запускаем таймер, который будет рисовать игру

//Начать новую игру при клике на СТАРТ
...
//Завершение игры
function gameOver() {
  context.clearRect(0, 0, fieldSize.x, fieldSize.y); //очистка заданной области пикселей
  clearInterval(game); //Отмена многокрастного повторения действия game
  showHiddenRecord();
}
//Окно с очками и кнопками после завершения игры
...
function showRules() {
  if (!isRulesShow) {
    rules.style.visibility = 'visible'; //отображает элемент как видимый
    rules.style.height = '550px';
    isRulesShow = true;
    return;
  }
  if (isRulesShow) {
    rules.style.visibility = 'hidden'; //элемент становится прозрачным
...
const AJAX_PROJECT_NAME = 'Snake_game';
...
function preSaveNewChamp() {
  ...
  if (!nameValue) {
    userNameInput.style.background = '#96c66d';
    userNameInput.focus();
    return;
  }
